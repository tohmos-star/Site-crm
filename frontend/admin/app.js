...
// Тарифы/зоны-как-вкладка убраны в архив (базовый биллинг без тарифов —
// см. frontend/ARCHIVED_SECTIONS.md). ZONES_CACHE остаётся: устройства
// всё ещё привязаны к зоне (см. populateZoneSelects в разделе "Устройства").
async function loadCommonCaches() {
  const clubsRes = await adminFetch('/api/clubs');
  const clubs = await clubsRes.json();
  CLUB_ID = clubs[0]?.id || null;

  const zonesRes = await adminFetch(`/api/zones?clubId=${CLUB_ID}`);
