...
echo "==> Поднимаю app"
# При recreate compose временно переименовывает старый контейнер (хэш-префикс)
# перед удалением; если предыдущий деплой прервался между переименованием и
# удалением, этот осиротевший контейнер остаётся и блокирует новый recreate
# конфликтом имени. rm -f -s по labels compose найдёт его независимо от
# текущего (временного) имени и уберёт перед up.
$SUDO docker compose rm -f -s app || true
